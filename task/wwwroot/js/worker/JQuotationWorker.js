...
function createOptionsSections(status,quoteid) {
    ...
    if (status == 'Pending') {
        td += '<a class="dropdown-item text-success" onclick="ChangeQuoteStatus(' + quoteid + ')" href="#!">Approve</a>';
    }

    td += '<a class="dropdown-item text-info" href="#!">Generate Agreement</a>';
...
onmessage = function (quotationArray) {
    ...
    for (var i = 0; i < quotationArray.data.length; i++) {
        console.log(quotationArray.data);
        ...
        tr += createOptionsSections(item.QuotationStatus,item.ID);
        tr += '</tr>';
